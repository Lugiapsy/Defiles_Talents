...
/**
 * Classe permettant de gérer la vue relative à la table tal_zone.
 *
 * @author giraudeaup
 */
public class V_Zone_All extends javax.swing.JFrame {
     ...
     * @param controller
     */
    public V_Zone_All(Controller controller) {
        this.controller = controller;
        initComponents();
        setTitle("Paramètres > Zones");
    }
    ...
    private void initComponents()
    {
        ...
        if (tableZone.getColumnModel().getColumnCount() > 0)
        {
            tableZone.getColumnModel().getColumn(0).setMinWidth(0);
            tableZone.getColumnModel().getColumn(0).setPreferredWidth(0);
            tableZone.getColumnModel().getColumn(0).setMaxWidth(0);
        }
    ...
    {//GEN-HEADEREND:event_btAddActionPerformed
        controller.getZoneEditFrame(V_Main.MODE_AJOUT).setVisible(true);
    ...
    {//GEN-HEADEREND:event_btEditActionPerformed
        if (tableZone.getSelectedRow() != -1) {
            controller.getZoneEditFrame((int) tableZone.getModel().getValueAt(tableZone.getSelectedRow(), 0), V_Main.MODE_EDITION).setVisible(true);
        }
    ...
    {//GEN-HEADEREND:event_btDelActionPerformed
        if (tableZone.getSelectedRow() != -1) {
            controller.getDeleteConfirmFrame("zone", String.valueOf((int) tableZone.getModel().getValueAt(tableZone.getSelectedRow(), 0))).setVisible(true);
        }
    ...
    private void setTable() {
        ...
        int size = controller.getZones().size();
        int i = 0;
        String classe = "null";
        ...
        for (M_Zone z : controller.getZones()) {
            model.setValueAt(z.getId(), i, 0);
            model.setValueAt(z.getLibelle(), i, 1);
            classe = ((controller.getClassePrix(z.getIdClasse()) != null) ? controller.getClassePrix(z.getIdClasse()).getLibelle() : "null");
